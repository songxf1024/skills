...
- 把目标路径集合写到 `.git/.edit-snapshot/paths.list`
...
输出里会包含会话 ID、记录时间、当前状态、备注，以及对应的 PRE 和 POST 提交摘要，方便在有多个会话时快速定位要查看或回退的是哪一次。
...
它不再单独展示 PRE 信息，而是直接输出当前文件或当前目标路径的最近 5 次记录。这样更适合在任务结束后直接贴给用户看，也更符合“先看当前文件最近记录”的使用习惯。
...
- 用 `git diff PRE..POST -- <paths...>` 查看本次保护路径范围内的改动
...
第二，未被 `.gitignore` 忽略的新增文件、生成物、二进制文件，甚至误放进目标路径内的敏感文件，也可能一起进入快照。因此在生成目录或产物目录被显式列为目标路径时，最好先确认 `.gitignore` 是否合理。
