...
File Change Guard uses a dedicated gitignore-style exclusion file instead of embedding default patterns in `SKILL.md`.
...
最近会话:
<sessions output with id + comment>
...
Users can edit this file directly to control what File Change Guard must exclude.
...
if [ -n "$ACTIVE_GUARD_IGNORE" ]; then
  for p in "${TARGETS[@]}"; do
    if git -c core.excludesFile="$ACTIVE_GUARD_IGNORE" check-ignore --no-index -q -- "$p"; then
      echo "Refusing because target path is excluded by active file-change-guard rules: $p" >&2
      exit 1
    fi
  done
fi
